...
namespace MCWebHogar.ControlPedidos
{
    public partial class DetalleDevolucion : System.Web.UI.Page
    {
        ...
        public void cargarProductosDevolucion()
        {
            ...
            if (Result != null && Result.Rows.Count > 0)
            {
                ...
                else
                {
                    ...
                    UpdatePanel_ListaProductosDevolucion.Update();
                    DGV_DetalleDevolucion.DataSource = Result;
                    DGV_DetalleDevolucion.DataBind();
                    UpdatePanel_DetalleDevolucion.Update();
                    string script = "estilosElementosBloqueados();cargarFiltros();";
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "ServerScriptcargarProductosDevolucion", script, true);
                }
            }
        ...
        protected void BTN_AbrirModalDetalleDevolucion_Click(object sender, EventArgs e)
        {
            cargarProductosDevolucion();
            string printer = Session["Printer"].ToString().Trim();
            TXT_NombreImpresora.Text = printer;
            UpdatePanel_DetalleDevolucion.Update();
            string script = "abrirModalDetalleDevolucion();estilosElementosBloqueados();cargarFiltros();";
            ScriptManager.RegisterStartupScript(this, this.GetType(), "ServerScriptBTN_AbrirModalDetalleDevolucion_Click", script, true);
        }

        protected void BTN_ImprimirDetalleDevolucion_Click(object sender, EventArgs e)
        {
            string fechaDevolucion = TXT_FechaDevolucion.Text;
            decimal monto = Convert.ToDecimal(TXT_MontoDevolucion.Text);
            string montoDevolucion = string.Format("{0:n0}", monto);
            string printer = TXT_NombreImpresora.Text.Trim();
            string script = "estilosElementosBloqueados();imprimir('" + montoDevolucion + "', '" + fechaDevolucion + "', '" + TXT_CodigoDevolucion.Text + "', '" + printer + "');";
